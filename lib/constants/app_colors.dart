import 'dart:ui';

class AppColors {
  static const Color PRIMARY_COLOR = Color(0xFF845EC2);
  static const Color ACCENT_COLOR = Color(0xFF0CA3E1);
  static const Color DARK_COLOR = Color(0xFF2A2A2A);
  static const Color PLACEHOLDER_COLOR = Color(0xFFE4E4E4);
  static const Color DARK_PLACEHOLDER = Color(0xFF373737);

  // For Shimmer Loading Effect
  static Color shimmerHighlightColor = Color(0xFF845EC2).withOpacity(0.6);
  static Color shimmerBaseColor = Color(0xFFC1BDCA);
}
